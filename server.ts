...
app.post("/api/upload-map", (req, res) => {
  try {
    ...
    // Also write it to the static asset path
    const targetAssetPath = path.join(process.cwd(), "src/assets/images/forena_actual_map_png_1779778690378.png");
    try {
      const base64Data = base64.replace(/^data:image\/\w+;base64,/, "");
      fs.writeFileSync(targetAssetPath, Buffer.from(base64Data, "base64"));
      console.log("[Server] Successfully wrote updated image to source assets.");
    } catch (e) {
      console.error("[Server] Note: could not write directly to asset path:", e);
    }
...
app.post("/api/upload-community", (req, res) => {
  try {
    ...
    // Also write it to the static asset path
    const targetAssetPath = path.join(process.cwd(), "src/assets/images/regenerated_image_1779789174361.jpg");
    try {
      const base64Data = base64.replace(/^data:image\/\w+;base64,/, "");
      fs.writeFileSync(targetAssetPath, Buffer.from(base64Data, "base64"));
      console.log("[Server] Successfully wrote updated community image to source assets.");
    } catch (e) {
      console.error("[Server] Note: could not write directly to community asset path:", e);
    }
